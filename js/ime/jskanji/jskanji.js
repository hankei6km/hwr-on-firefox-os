...
(function() {
  ...
  var IMEngine = function ime() {
    ...
    var _handwritingPadEnabled = false;

    var _isKeyboardOverlay = false;
    ...
    this.hwr = function ime_hwr(result) {
      overlayKeyboard(result.results);
    };
    ...
    var handleNormalKey = function ime_handleNormalKey(code) {
      ...
      if (_handwritingPadEnabled) {
        var isHiragana = false;
        var clen = IMEHiraganaCycleTable.length;
        for (var cidx=0; cidx<clen; cidx++) {
          if (IMEHiraganaCycleTable[cidx].indexOf(kana) >= 0) {
            isHiragana = true;
          }
        }
        if (!isHiragana) {
          _glue.sendString(SyllableUtils.arrayToString(_inputBuf) + kana);
    ...
    var handleSpecialKey = function ime_handleSpecialKey(code) {
      ...
      switch (code) {
        ...
        case IMESpecialKey.CASE_CHANGE:
          debug('case change');
          if (_inputBuf.length === 0) {
            _handwritingPadEnabled = !_handwritingPadEnabled;
            _glue.setHandwritingPad(_handwritingPadEnabled);
            if (!_handwritingPadEnabled) {
              restoreKeyboard('jp-kanji');
            }
            break;
          }
        ...
        case KeyEvent.DOM_VK_BACK_SPACE:
          if (_handwritingPadEnabled && _isKeyboardOverlay) {
            restoreKeyboard('jp-kanji');
          }else{
            handleBackspace();
          }
    ...
    var recoverLists = function ime_resetLists() {
      var id = null;
      return function(){
        if (id) {
          clearTimeout(id);
        }
        var inputBufSaved = _inputBuf;
        var candidateListSaved = _candidateList; 
        
        id = setTimeout(function() {
          _inputBuf = inputBufSaved;
          var bufStr = SyllableUtils.arrayToString(_inputBuf);
          _glue.sendPendingSymbols(bufStr);
          _candidateList = candidateListSaved; 
          _glue.sendCandidates(_candidateList);
    ...
    var recoverListsHandle = recoverLists();

    var overlayKeyboard = function ime_overlayKeyboard(results) {
      _isKeyboardOverlay = true;
      recoverListsHandle();
      _glue.overlayKeyboard('jp-kanji', [
                            {line:0,pos:1,item:{ value: results[0].value, ratio: 2 }},
                            {line:0,pos:2,item:{ value: results[1].value, ratio: 2 }},
                            {line:0,pos:3,item:{ value: results[2].value, ratio: 2 }},
                            {line:1,pos:1,item:{ value: results[3].value, ratio: 2 }},
                            {line:1,pos:2,item:{ value: results[4].value, ratio: 2 }},
                            {line:1,pos:3,item:{ value: results[5].value, ratio: 2 }},
                            {line:2,pos:2,item:{ value: results[6].value, ratio: 2 }},
                            {line:2,pos:3,item:{ value: results[7].value, ratio: 2 }},
                            {line:2,pos:4,item:{ value: results[8].value, ratio: 2 }},
                            {line:3,pos:3,item:{ value: results[9].value, ratio: 2 }},
      ]);
    };

    var restoreKeyboard = function ime_restoreKeyboard(keyboardName) {
      _isKeyboardOverlay = false;
      recoverListsHandle();
      _glue.restoreKeyboard(keyboardName);
    };
