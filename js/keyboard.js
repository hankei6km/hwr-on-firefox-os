...
var handwritingPadEnabled = false;
...
// Context used to The Handwriting pad
var handwritingPadCtx = null;
var handwritingPadStrokeIdx = -1;
var handwritingPadStroke = null;
var handwritingPadStrokeTbl = new Array(0);
var handwritingPadSocket = null;
...
function initKeyboard() {
  ...
  // Connect to the hwr server
  if (handwritingPadSocket == null) {
    handwritingPadSocket = io.connect('http://localhost:3000');
    handwritingPadSocket.on('result', onPadResult);
  }
  // Attach event listeners to the HandwritingPad element
  IMERender.pad.addEventListener('mousedown', onPadMouseDown);
  document.addEventListener('mousemove', onPadMouseMove);
  document.addEventListener('mouseup', onPadMouseUp);
...
function renderKeyboard(keyboardName) {
  ...
  function drawKeyboard() {
    ...
    IMERender.draw(currentLayout, {
      ...
      handwritingPadEnabled: handwritingPadEnabled
    });
    ...
    // Initialize the handwriting pad
    if (handwritingPadEnabled) {
      var canvas = document.getElementById("handwriting-pad-canvas");
      handwritingPadCtx = canvas.getContext('2d');
      redrawStrokes();
      canvas = null;
      IMERender.pad.classList.remove('handwriting-pad-disabled');
      IMERender.pad.classList.add('handwriting-pad-enabled');
    } else {
      handwritingPadCtx = null;
      IMERender.pad.classList.remove('handwriting-pad-enabled');
      IMERender.pad.classList.add('handwriting-pad-disabled');
    }
  }
...
function sendDelete(isRepeat) {
  if (handwritingPadEnabled && handwritingPadStrokeIdx >= 0) {
    clearHandwritingPad();
  }
...
function onPadMouseDown(evt) {
  if (handwritingPadEnabled) {
    evt.preventDefault();
    handwritingPadCtx.beginPath();
    handwritingPadStrokeIdx++;
    handwritingPadStroke = new Array(0);
    handwritingPadCtx.moveTo(evt.pageX, evt.pageY);
    storeHandwritingPadStroke(evt.pageX, evt.pageY);
  }
}

function onPadMouseMove(evt) {
  if (handwritingPadStroke) {
    evt.preventDefault();
    storeHandwritingPadStroke(evt.pageX, evt.pageY);
  }
}

function onPadMouseUp(evt) {
  if (handwritingPadStroke) {
    evt.preventDefault();
    handwritingPadSocket.emit('add', {
      idx: handwritingPadStrokeIdx,
      nbest: 10,
      pt: handwritingPadStroke});
    handwritingPadCtx.stroke();
    handwritingPadStrokeTbl.push(handwritingPadStroke);
    handwritingPadStroke = null;
  }
}

function onPadResult(data) {
  if (handwritingPadStrokeIdx <= data.idx) {
    if (inputMethod.hwr) {
      inputMethod.hwr(data);
    }
  }
}
...
function clearHandwritingPad() {
  if (handwritingPadEnabled) {
    ...
    handwritingPadSocket.emit('clear', {
      width: window.innerWidth,
      height: window.innerHeight - IMERender.getHeight()
    });
    handwritingPadCtx.clearRect(0, 0, w, h);
    handwritingPadStrokeIdx = -1;
    handwritingPadStrokeTbl = new Array(0);
    IMERender.showCandidates([]);
  }
}

function storeHandwritingPadStroke(x, y) {
    handwritingPadStroke.push({x:x, y:y})
    handwritingPadCtx.lineTo(x, y);
    handwritingPadCtx.moveTo(x, y);
}

function redrawStrokes() {
  for (var sidx = 0; sidx < handwritingPadStrokeTbl.length; sidx++) {
    handwritingPadCtx.beginPath();
    for (var pidx = 0; pidx < handwritingPadStrokeTbl[sidx].length - 1; pidx++) {
      handwritingPadCtx.moveTo(
        handwritingPadStrokeTbl[sidx][pidx].x,
        handwritingPadStrokeTbl[sidx][pidx].y
      );
      handwritingPadCtx.lineTo(
        handwritingPadStrokeTbl[sidx][pidx+1].x,
        handwritingPadStrokeTbl[sidx][pidx+1].y
      );
    }
    handwritingPadCtx.stroke();
  }
...
function loadIMEngine(name) {
  ...
  var glue = {
    ...
    setHandwritingPad: function kc_glue_setHandwritingPad(v) {
      handwritingPadEnabled = v;
      renderKeyboard(keyboardName);
      if (handwritingPadSocket != null) {
        if (handwritingPadEnabled) {
          handwritingPadSocket.socket.reconnect();
        } else {
          handwritingPadSocket.disconnect();
        }
      }
      clearHandwritingPad();
    },
    clearHandwritingPad: clearHandwritingPad,
    overlayKeyboard: function kc_glue_overlayKeyboard(inKeyboardName, inKeys) {
      if (!(inKeyboardName in saveKeyboards)) {
        saveKeyboards[inKeyboardName] =
          JSON.parse(JSON.stringify(Keyboards[inKeyboardName]));
      }
      var len = inKeys.length;
      for (var idx=0; idx<len; idx++) {
        var key = inKeys[idx];
        Keyboards[inKeyboardName].keys[key.line][key.pos] = key.item;
      }
      renderKeyboard(keyboardName);
    },
    restoreKeyboard: function kc_glue_restoreKeyboard(inKeyboardName) {
      if (inKeyboardName in saveKeyboards) {
        Keyboards[inKeyboardName] =
          JSON.parse(JSON.stringify(saveKeyboards[inKeyboardName]));
      }
      renderKeyboard(keyboardName);
    }
  };
